...
export default function Customers() {
  ...
  function calculateTotal(formData = repairForm) {
    return (
  ...
  function updateRepairCharge(field, value) {
    const next = { ...repairForm, [field]: value }
    next.total = calculateTotal(next)
    setRepairForm(next)
  }

  function printRepairOrder(ro) {
    const customer = customers.find((c) => c.id === ro.customer_id)
    const vehicle = vehicles.find((v) => v.id === ro.vehicle_id)
    const isPickedUp = ro.status === 'Picked Up'

    const html = `
      <!doctype html>
      <html>
        <head>
          <title>${ro.ro_number || 'Repair Order'}</title>
          <style>
            ...
            .shop h1{margin:0;font-size:30px;letter-spacing:.02em}
            .shop p{margin:5px 0;color:#444}
            ...
            .box h2{font-size:15px;text-transform:uppercase;margin:0 0 10px;color:#111}
            .box p{margin:6px 0}
            .section{margin:18px 0}
            .section h2{font-size:16px;text-transform:uppercase;border-bottom:1px solid #d1d5db;padding-bottom:8px}
            .notes{white-space:pre-wrap;line-height:1.5;color:#222}
        ...
        <body>
          ...
          <div class="meta">
            <div class="box">
              <h2>Repair Order</h2>
              <p><strong>RO #:</strong> ${ro.ro_number || '-'}</p>
              <p><strong>Status:</strong> ${ro.status || '-'}</p>
              <p><strong>Technician:</strong> ${ro.technician || '-'}</p>
              <p><strong>Opened:</strong> ${ro.opened_at ? new Date(ro.opened_at).toLocaleDateString() : '-'}</p>
            </div>
            ...
            <div class="box">
              ...
              <p><strong>Mileage In:</strong> ${ro.mileage_in || '-'}</p>
            </div>

            <div class="box">
              <h2>Charges</h2>
              <p><strong>Parts:</strong> $${money(ro.parts_total)}</p>
              <p><strong>Labor:</strong> $${money(ro.labor_total)}</p>
              <p><strong>Tax:</strong> $${money(ro.tax)}</p>
              <p><strong>Total:</strong> $${money(ro.total)}</p>
            </div>
          </div>

          <div class="section">
            <h2>Customer Complaint</h2>
            <div class="notes">${ro.customer_complaint || '-'}</div>
          </div>

          <div class="section">
            <h2>Diagnosis</h2>
            <div class="notes">${ro.diagnosis || '-'}</div>
          </div>

          <div class="section">
            <h2>Repairs Performed</h2>
            <div class="notes">${ro.repairs_performed || '-'}</div>
          </div>
          ...
          <div class="section">
            ...
            <table>
              ...
              <tbody>
                <tr><td>Parts</td><td>$${money(ro.parts_total)}</td></tr>
                <tr><td>Labor</td><td>$${money(ro.labor_total)}</td></tr>
                <tr><td>Tax</td><td>$${money(ro.tax)}</td></tr>
              </tbody>
            </table>

            <div class="totals">
              <div><span>Parts</span><strong>$${money(ro.parts_total)}</strong></div>
              <div><span>Labor</span><strong>$${money(ro.labor_total)}</strong></div>
              <div><span>Tax</span><strong>$${money(ro.tax)}</strong></div>
              <div class="grand"><span>Total</span><strong>$${money(ro.total)}</strong></div>
            </div>
          </div>
  ...
  function openRepairForm(ro = null, vehicleId = '') {
    if (ro) {
      ...
      setRepairForm({
        ...
        partsTotal: ro.parts_total || '',
        laborTotal: ro.labor_total || '',
        tax: ro.tax || '',
        total: ro.total || '',
  ...
  async function saveRepairOrder() {
    ...
    const payload = {
      ...
      parts_total: Number(repairForm.partsTotal || 0),
      labor_total: Number(repairForm.laborTotal || 0),
      tax: Number(repairForm.tax || 0),
      total: Number(repairForm.total || calculateTotal()),
        ...
        <div className="modalCard wideModal">
          ...
          <div className="modalGrid">
            <input
              ...
              value={repairForm.partsTotal}
              onChange={(e) => updateRepairCharge('partsTotal', e.target.value)}
            />
            ...
            <input
              ...
              value={repairForm.laborTotal}
              onChange={(e) => updateRepairCharge('laborTotal', e.target.value)}
            />
            ...
            <input
              ...
              step="0.01"
              placeholder="Tax"
              value={repairForm.tax}
              onChange={(e) => updateRepairCharge('tax', e.target.value)}
            />
            ...
            <input
              ...
              value={repairForm.total}
              onChange={(e) => setRepairForm({ ...repairForm, total: e.target.value })}
            />
          </div>

          {(repairForm.status === 'Completed' || repairForm.status === 'Picked Up') && (
            <p className="printNotice">PDF will be available once this repair order is saved.</p>
          )}
  ...
  return (
    <>
      ...
      <div className="workspaceGrid stackedWorkspace">
        <section className="profilePanel customersPanel">
          <div className="panelTitle">
            <span>👥</span>
            <h2>Customers</h2>
          </div>
